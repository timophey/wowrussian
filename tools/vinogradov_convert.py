#!/usr/bin/env python3
"""
Конвертер орфографического словаря Виноградова из PDF в JSON и Excel.
Извлекает записи словаря, начиная со страницы 9.
...
def extract_dictionary(pdf_path, start_page=9, max_pages=None, max_entries=None):
    """
...
def main():
    ...
    if not pdf_path.exists():
        print(f"Ошибка: файл не найден: {pdf_path}")
        print(f"Убедитесь, что orfograficheskij_slovar.pdf находится в папке tools/")
        return 1
    ...
    # Сохраняем в JSON
    json_path = base_dir / 'vinogradov_dictionary.json'
    save_to_json(entries, json_path)

    # Сохраняем в Excel
    excel_path = base_dir / 'vinogradov_dictionary.xlsx'
